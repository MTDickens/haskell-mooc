...
instance Functor List where
  fmap f (LNode head rest) = LNode (f head) (fmap f rest)
  fmap f Empty             = Empty
...
instance Functor TwoList where
  fmap f (TwoNode x1 x2 rest) = TwoNode (f x1) (f x2) (fmap f rest)
...
count x = foldr f 0
  where f a b = if a == x then (b + 1) else b
...
foldableFind elem fold = foldr (\foldElem check -> check || foldElem == elem) False fold
inBoth foldA foldB = foldr fA [] foldA
  where fA elem li = if foldableFind elem foldB then elem:li else li
...
instance Foldable List where
  ...
  foldr f start (LNode head rest) = f head (foldr f start rest)
...
instance Foldable TwoList where
  foldr f start (TwoNode x1 x2 rest) = f x1 (f x2 (foldr f start rest))
  foldr f start TwoEmpty                = start
