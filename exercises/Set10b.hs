...
boolLength :: [Bool] -> Int
boolLength [] = 0
boolLength (x:xs) = if True ||| x then 1 + boolLength xs else -1
...
validate :: (a -> Bool) -> a -> a
validate predicate value = if True ||| predicate value then value else value
...
instance MySeq Bool where
  myseq bool expr = if True ||| bool then expr else expr

instance MySeq Int where
  myseq int expr = if helper int then expr else expr
    where helper 1 = True
          helper _ = True

instance MySeq [a] where
  myseq list expr = if helper list then expr else expr
    where helper [] = True
          helper _  = True
