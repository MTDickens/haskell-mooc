...
instance Eq Name where
  (==) (Name xs) (Name ys) = (map Data.Char.toLower xs) == (map Data.Char.toLower ys) 
...
instance Eq a => Eq (List a) where
  (==) Empty Empty = True
  (==) (LNode xf xr) (LNode yf yr) = (xf == yf) && (xr == yr)
  (==) _ _ = False
...
mygcd 0 m = m
mygcd n 0 = n
mygcd n m = if n < m then mygcd m n
                     else mygcd m (mod n m)
simplify :: RationalNumber -> RationalNumber
simplify (RationalNumber n m) = RationalNumber (n `div` g) (m `div` g)  where g = mygcd (abs n) (abs m)
...
instance Num RationalNumber where
  ...
  signum (RationalNumber x y) = RationalNumber (signum x) (signum y)
