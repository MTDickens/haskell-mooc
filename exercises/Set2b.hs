...
myGcd a b = case smaller of 0 -> bigger
                            smaller -> myGcd (bigger `mod` smaller) smaller
    where smaller = if a < b then a else b
          bigger = if a < b then b else a
...
leftpad str enough
    | (length str) < enough = leftpad (" " ++ str) enough
    | otherwise = str
...
countdown sec = "Ready! " ++ counthelper sec ++ "Liftoff!"
counthelper :: Integer -> String
counthelper 0 = ""
counthelper sec = show sec ++ "... " ++ counthelper (sec - 1)
...
smallestDivisor num = divHelper num 2
divHelper :: Integer -> Integer -> Integer
divHelper num div = if (num `mod` div) == 0 then div
                                            else divHelper num (div + 1)
...
isPrime num = if num <= 1 then False else (smallestDivisor num == num)
...
biggestPrimeAtMost num = if isPrime num then num else biggestPrimeAtMost (num - 1)
