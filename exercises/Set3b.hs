...
sumHelper n i = if n > i then [] else ((n+1)*n`div`2):sumHelper (n+1) i
...
indexDefault (h:t) n def = indexDefault t (n-1) def
...
sumsOf :: [Int] -> [Int]
sumsOf xs = sumHelper2 0 xs
sumHelper2 _ [] = []
sumHelper2 add (h:t) = newadd:sumHelper2 newadd t
    where newadd = add + h
...
merge [] ys = ys
merge xs [] = xs
merge (xsh:xst) (ysh:yst) = if xsh < ysh
                            then xsh:(merge xst (ysh:yst))
                            else ysh:(merge yst (xsh:xst))
...
mymaximum bigger initial (h:t) = mymaximum bigger (if (bigger initial h) then initial else h) t
...
maybeMap f [] = []
maybeMap f (h:t) = case (f h) of (Just x) -> x:(maybeMap f t)
                                 _ -> maybeMap f t
