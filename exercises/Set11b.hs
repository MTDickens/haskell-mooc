...
hFetchLines hdl = do
  checkEOF <- hIsEOF hdl
  if checkEOF then return []
              else do
    str <- hGetLine hdl
    rest <- hFetchLines hdl
    return $ str : rest
--  strs <- hGetContents hdl
--  return $ lines strs
...
hSelectLines h nums = go h 1
  where go hdl lineNum = do
          checkEOF <- hIsEOF hdl
          if checkEOF then return []
                      else do
          str <- hGetLine hdl
          rest <- go hdl (lineNum + 1)
          return $ if elem lineNum nums then str : rest else rest
