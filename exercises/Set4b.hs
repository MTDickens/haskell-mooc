...
countHelper a b = maybe 1 (const 0) a + b
...
slStart = (0.0,0)
slHelper addFrom addTo = (fst addTo + addFrom, snd addTo + 1)
...
largestHelper x largeList
  | null largeList = [x]
  | otherwise = case compare x (head largeList) of
      EQ -> x:largeList
      LT -> largeList
      GT -> [x]
...
lastHelper _ original  = original
