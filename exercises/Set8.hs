...
paintSolid color (Shape f) (Picture ff) = Picture (\coord -> if f coord then color else ff coord)
------------------------------------------------------------------------------
...
triple :: Color -> Color -> Color -> Picture
triple a b c = Picture f
  where f (Coord x y)
          | mod (x+y) 3 == 0 = a
          | mod (x+y) 3 == 1 = b
          | otherwise        = c

tripleStripes :: Color -> Color -> Color -> Picture
tripleStripes a b c = Picture f
  where f (Coord x y)
          | mod (y `div` 10) 3 == 0 = a
          | mod (y `div` 10) 3 == 1 = b
          | otherwise        = c
...
paint (Picture f1) (Shape g) (Picture f2) = Picture (\coord -> (if g coord then f1 else f2) coord)
...
instance Transform Fill where
  apply (Fill c) _ = Picture (\coord -> c)
...
instance Transform Blur where
  apply Blur (Picture f) = Picture (\coord@(Coord x y) -> Color (r $ colors x y) (g $ colors x y) (b $ colors x y))
    where r = ((`div` 5).sum.map (\(Color r g b)-> r))
          g = ((`div` 5).sum.map (\(Color r g b)-> g))
          b = ((`div` 5).sum.map (\(Color r g b)-> b))
          colors x y = (map f.map (\(a,b)->Coord (x+a) (y+b))) [(0,0),(0,-1),(-1,0),(0,1),(1,0)]
