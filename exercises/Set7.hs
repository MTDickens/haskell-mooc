...
member item (Set xs) = elem item xs
...
data State = Start | Error | Finished | AddEggsFinished
  | AddSugarFinished | AddFlourFinished | FlourSugarFinished | MixFinished
    deriving (Eq,Show)

step :: State -> Event -> State
step Start AddEggs = AddEggsFinished
step AddEggsFinished AddFlour = AddFlourFinished
step AddEggsFinished AddSugar = AddSugarFinished
step AddFlourFinished AddSugar = FlourSugarFinished
step AddSugarFinished AddFlour = FlourSugarFinished
step FlourSugarFinished Mix = MixFinished
step MixFinished Bake = Finished
...
passwordAllowed str (ContainsSome chars) = any (\x -> elem x str) chars
passwordAllowed str (DoesNotContain chars) = not $ passwordAllowed str (ContainsSome chars)
passwordAllowed str (And rx ry) = (passwordAllowed str rx) && (passwordAllowed str ry)
passwordAllowed str (Or rx ry) = (passwordAllowed str rx) || (passwordAllowed str ry)
...
render (Num n) = show n
render (Add n m) = "(" ++ (render n) ++ "+" ++ (render m) ++ ")"
render (Mul n m) = "(" ++ (render n) ++ "*" ++ (render m) ++ ")"
