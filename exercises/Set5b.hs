...
mapTree f (Node this l r) = Node (f this) (mapTree f l) (mapTree f r)
...
isOrdered (Node this l r) = (allValues (<this) l) && (allValues (>this) r) && isOrdered l && isOrdered r
...
set (x:xs) val (Node a l r) = (Node a (case x of StepL -> (set xs val l)
                                                 StepR -> l)
                                      (case x of StepL -> r
                                                 StepR -> (set xs val r)))
...
search val (Node this l r) = if this == val then (Just [])
                                            else (addPath (search val l) (search val r))
  where addPath Nothing (Just xs) = (Just (StepR:xs))
        addPath (Just xs) Nothing = (Just (StepL:xs))
        addPath Nothing Nothing   = Nothing
