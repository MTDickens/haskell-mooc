...
buyOneMore (MkShoppingEntry good price amount) = MkShoppingEntry good price (amount + 1)
...
setName hisName p = MkPerson {name = hisName, age = age p}

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge hisAge p = MkPerson {name = name p, age = hisAge}
...
fromList [] = Empty
fromList ((key,value):next) = Pair key value (fromList next)
...
fromNat :: Nat -> Int
fromNat Zero = 0
fromNat (PlusOne z) = 1 + fromNat z

toNat :: Int -> Maybe Nat
toNat z = case compare z 0 of LT -> Nothing
                              EQ -> Just Zero
                              GT -> Just (PlusOne $ maybe Zero id (toNat (z - 1)))
...
fromBin :: Bin -> Int
fromBin End = 0
fromBin (I next) = 1 + 2 * (fromBin next)
fromBin (O next) = 0 + 2 * (fromBin next)

toBin :: Int -> Bin
toBin 1 = (I End)
toBin 0 = (O End)
toBin n = case n `mod` 2 of 0 -> (O (toBin remain))
                            1 -> (I (toBin remain))
  where remain = n `div` 2
